...
if ~isempty(Triggers)
    ...
    %% if available, load lookup table and convert trigger values (type) 
    %% back to string
    
    % find path to containing folder, build path to lookup csv file, and
    % attempt to load
    [pth, ~, ~] = fileparts(EEG.filepath);
    file_lookup = fullfile(pth, 'enobio_markers.csv');
    if ~exist(file_lookup, 'file') 
        warning('Could not find enobio_markers_lookup.csv in the same folder as the data file. Markers will not be translated back to text.')
    else
        % load
        try
            ...
            % replace .easy events with those from lookup file
            type_converted = tab_trig.Var1;
            % strip trailing zeros from events
            type_converted = strrep(type_converted, '.00000000000000000000000000000000000000000000000000', '');
        catch ERR
            error('Error reading lookup table. Error message was:\n\n%s', ERR.message)
        end
        fprintf('\n\n<strong>Events were converted from numeric to text.</strong>\n')
        fprintf('\t- Used lookup file: %s\n', file_lookup)
        fprintf('\t- Markers from the .easy file are stored in EEG.event_numeric.\n')
        fprintf('\t- This is how numeric events were mapped to text:\n')
        disp(cell2table([type, type_converted], 'variablenames', {'Numeric', 'Text'}))
    end
    
    EEG.event = struct('type',type_converted,'latency',lat,'latency_ms',lat_ms);
    EEG.event_numeric = struct('type',type,'latency',lat,'latency_ms',lat_ms);
